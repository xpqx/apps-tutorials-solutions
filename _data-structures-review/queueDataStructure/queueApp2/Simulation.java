package queueDataStructure.queueApp2;
...
public class Simulation {
	...
	public Simulation() {
		...
		System.out.println("Mean interarrival time:" + MEAN_ARRIVAL_TIME);
		for (int i = 0; i < SERVERS; i++) {
			double meanServiceTime = randomService.nextDouble();
			servers[i] = new Server(i, meanServiceTime);
			System.out.printf("Mean service time for %s: %4.1f%n", servers[i], servers[i].getMeanServiceTime());
		}
